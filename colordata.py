...
class colordata(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        recon_const_outres = np.zeros(
            (1, self.outshape[0], self.outshape[1]), dtype="f"
        )
    ...
    def __getweights__(self, img):
        ...
        binweights = np.array(
            [self.lossweights[v1][v2] for v1, v2 in zip(binid_a, binid_b)]
        )
    ...
    def saveoutput_gt(
        self, net_op, gt, prefix, batch_size, num_cols=8, net_recon_const=None
    ):

        net_out_img = self.__tiledoutput__(
            net_op, batch_size, num_cols=num_cols, net_recon_const=net_recon_const
        )

        gt_out_img = self.__tiledoutput__(
            gt, batch_size, num_cols=num_cols, net_recon_const=net_recon_const
        )
        ...
        cv2.imwrite(
            out_fn_pred, np.concatenate((net_out_img, border_img, gt_out_img), axis=1)
        )
    ...
    def __tiledoutput__(self, net_op, batch_size, num_cols=8, net_recon_const=None):
        ...
        num_rows = np.int_(np.ceil((batch_size * 1.0) / num_cols))
        out_img = np.zeros(
            (num_rows * self.outshape[0], num_cols * self.outshape[1], 3), dtype="uint8"
        )
        ...
        for i in range(batch_size):
            if i % num_cols == 0 and i > 0:
                r = r + 1
                c = 0
            img_lab[..., 0] = self.__decodeimg__(
                net_recon_const[i, 0, :, :].reshape(self.outshape[0], self.outshape[1])
            )
            img_lab[..., 1] = self.__decodeimg__(
                net_op[i, 0, :, :].reshape(self.shape[0], self.shape[1])
            )
            img_lab[..., 2] = self.__decodeimg__(
                net_op[i, 1, :, :].reshape(self.shape[0], self.shape[1])
            )
